...
if (typeof window !== 'undefined') {
  ...
  (window as any).__AI_API__ = {
    ...
    // 디버그: 합성 가능 레시피 확인
    getRecipes: () => {
      return Object.keys(UNIT_RECIPES).map(targetId => {
        const check = checkCraftability(targetId);
        return {
          target: targetId,
          targetName: UNIT_MAP[targetId]?.name,
          isCraftable: check.isCraftable,
          ingredients: check.ingredients.map(i => `${UNIT_MAP[i.id]?.name ?? i.id}:★${i.star}${i.owned ? '✅' : '❌'}`),
        };
      }).filter(r => r.isCraftable);
    },
    // 디버그: 유닛이 참여하는 레시피 역조회
    getRecipesFor: (unitId: string) => {
      return getRecipesForUnit(unitId).map(tid => ({
        target: tid,
        name: UNIT_MAP[tid]?.name,
        craftable: checkCraftability(tid).isCraftable,
      }));
    },
    // 디버그: 합성 모달 열기
    openCraftModal: (unitId: string) => showCraftModal(unitId),
  };
}
...
// ── 레시피 합성 엔진 (Interactive Craft Modal) ────────────────
...
/** 특정 유닛이 재료로 사용되는 모든 레시피 역방향 조회 */
function getRecipesForUnit(baseUnitId: string): string[] {
  const results: string[] = [];
  for (const [targetId, reqs] of Object.entries(UNIT_RECIPES)) {
    const hasIngredient = reqs.some(r => parseRecipeReq(r).id === baseUnitId);
    if (hasIngredient) results.push(targetId);
  }
  return results;
}

/** 레시피 합성 가능 여부 상세 체크 */
function checkCraftability(targetId: string): {
  isCraftable: boolean;
  ingredients: { id: string; star: number; owned: boolean; ownedStar: number }[];
  unitsToConsume: UnitInstance[];
} {
  const p = player();
  const allUnits = [...p.board, ...p.bench];
  const reqs = UNIT_RECIPES[targetId];
  if (!reqs) return { isCraftable: false, ingredients: [], unitsToConsume: [] };

  const parsed = reqs.map(parseRecipeReq);
  const usedIds = new Set<string>();
  const ingredients: { id: string; star: number; owned: boolean; ownedStar: number }[] = [];
  const unitsToConsume: UnitInstance[] = [];
  ...
  for (const req of parsed) {
    ...
    if (candidate) {
      ...
      unitsToConsume.push(candidate);
      ingredients.push({ id: req.id, star: req.star, owned: true, ownedStar: candidate.star });
    } else {
      isCraftable = false;
      // 최고 별 보유 유닛 찾기 (상태 표시용)
      const best = allUnits.filter(u => u.unitId === req.id && !usedIds.has(u.instanceId))
        .sort((a, b) => b.star - a.star)[0];
      ingredients.push({ id: req.id, star: req.star, owned: false, ownedStar: best?.star ?? 0 });
    }
  }

  return { isCraftable, ingredients, unitsToConsume };
}
...
function renderCraftPanel(): void {
  ...
  for (const [targetId] of Object.entries(UNIT_RECIPES)) {
    ...
    if (!def) continue;
    const cost = def.cost;
    if (cost === 4) tierGroups[0].targets.push(targetId);
    else if (cost === 5) tierGroups[1].targets.push(targetId);
    else if (cost === 7) tierGroups[2].targets.push(targetId);
    else if (cost >= 10) tierGroups[3].targets.push(targetId);
  }
  ...
  for (const group of tierGroups) {
    ...
    const isOpen = craftTierExpanded[group.key] ?? false;
    const arrow = isOpen ? '▼' : '▶';

    // 티어 헤더: 합성 가능 개수 표시
    const craftableCount = group.targets.filter(tid => checkCraftability(tid).isCraftable).length;
    const craftBadge = craftableCount > 0
      ? `<span style="color:#00ff88;font-size:11px;margin-left:6px;">⚡${craftableCount}</span>`
      : '';

    html += `<div class="craft-tier-header" style="color:${group.color}" data-tier="${group.key}">
      <span>${arrow} ${group.label}${craftBadge}</span>
      <span style="font-size:11px;color:#64748b">${group.targets.length}개</span>
    </div>`;
    ...
    if (!isOpen) {
      ...
      for (const targetId of group.targets) {
        ...
        if (check.isCraftable) {
          ...
          html += `<span class="craft-progress">${owned}/${total}</span>`;
        }
        html += `</div>`;
      }
    } else {
      ...
      for (const targetId of group.targets) {
        ...
        if (isExpanded) {
          html += `<div class="craft-recipe-detail">`;
          ...
          if (check.isCraftable) {
            html += `<button class="craft-btn" data-target="${targetId}">⚡ CRAFT</button>`;
          }
          html += `</div>`;
        }
  ...
  if (!html) {
    container.innerHTML = '<div style="font-size:12px;color:#94a3b8;padding:8px;text-align:center;">레시피 데이터 없음</div>';
    return;
  }
  ...
  // 티어 헤더 클릭 → 접기/펼치기
  container.querySelectorAll('.craft-tier-header').forEach(hdr => {
    hdr.addEventListener('click', () => {
      const tier = (hdr as HTMLElement).dataset.tier!;
      craftTierExpanded[tier] = !craftTierExpanded[tier];
      renderCraftPanel();
    });
  });

  // 레시피 헤더 클릭 → 상세 확장/축소
  container.querySelectorAll('.craft-recipe-header').forEach(hdr => {
    hdr.addEventListener('click', () => {
      const tid = (hdr as HTMLElement).dataset.target!;
      craftPanelExpandedRecipe = craftPanelExpandedRecipe === tid ? null : tid;
      renderCraftPanel();
    });
  });
